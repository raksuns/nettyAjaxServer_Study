...
public class Server {
	...
	public static final String DEFAULT_CONTENT_DIR = String.format(
			".%ssrc%smain%sresources%swww", File.separator, File.separator,
	 ...
	 *            <ol>
	 *            <li>The binding interface in the form of an IP address or a
	 *            host name</li>
	...
	protected Map<String, PipelineModifier> getPipelineModifiers() {
		...
		for (Iterator<URL> urlIter = ClasspathHelper.forClassLoader()
				.iterator(); urlIter.hasNext();) {
			URL url = urlIter.next();
			if (url.toString().toLowerCase().endsWith(".jar")
					|| !url.toString().toLowerCase().contains(".")) {
				urls.add(url);
			}
		}
		...
		for (Class<?> clazz : ref.getTypesAnnotatedWith(URIHandler.class)) {
			if (PipelineModifier.class.isAssignableFrom(clazz)) {
				...
				try {
					PipelineModifier pm = (PipelineModifier) clazz
							.newInstance();
					String[] names = uhandler.uri();
					for (String name : names) {
						name = name.trim().toLowerCase();
						if (map.containsKey(name)) {
							LOG.warn("The handler ["
									+ pm.getName()
									+ "] offering URI ["
									+ name
					...
					LOG.error("Failed to create PipelineModifier instance from class ["
							+ clazz.getName() + "]");
				}
			}
		}
